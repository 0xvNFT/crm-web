...
export function useInvoices(page = 0, size = 20, filters: Record<string, string> = {}) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      client
        .get<PagePharmaInvoice>('/api/pharma/invoices', {
...
export function useInvoiceSearch(q: string) {
  return useQuery({
    ...
    queryFn: () =>
      client
        .get<PharmaInvoice[]>('/api/pharma/invoices/search', { params: { q } })
        .then((r) => r.data),
...
export function useInvoice(id: string) {
  return useQuery({
    ...
    queryFn: () => client.get<PharmaInvoice>(`/api/pharma/invoices/${id}`).then((r) => r.data),
    enabled: !!id,
  })
}
...
export function useInvoicesByAccount(accountId: string, page = 0, size = 10) {
  return useQuery({
    ...
    queryFn: () =>
      client
        .get<PagePharmaInvoice>(`/api/pharma/invoices/by-account/${accountId}`, {
...
export function useCreateInvoice() {
  ...
  return useMutation({
    mutationFn: (data: CreateInvoiceRequest) =>
      client.post<PharmaInvoice>('/api/pharma/invoices', data).then((r) => r.data),
