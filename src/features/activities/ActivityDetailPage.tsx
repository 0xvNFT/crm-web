...
export function DetailField({ label, value }: { label: string; value?: string | number | boolean | null }) {
  const display =
...
export default function ActivityDetailPage() {
  ...
  // Type assertion needed to access API fields missing from local app-types.ts
  const activity = data as PharmaActivity & {
    scheduledAt?: string
    completedAt?: string
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <StatusBadge status={(activity.status ?? 'active').toUpperCase()} />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <DetailSection title="Links">
          <DetailField label="Contact ID" value={activity.contactId} />
          <DetailField label="Account ID" value={activity.accountId} />
          <DetailField label="User ID" value={activity.userId} />
        </DetailSection>
